...
import com.github.iamshijun.passport.util.CollectionUtils;
import com.github.iamshijun.passport.util.StringUtils;

/**
 * <p>文件来源自github上Maxime BOSSARD用户cas-slo-cluster项目中的
 * <a href="https://github.com/mxbossard/cas-slo-cluster/blob/master/src/org/esco/cas/client/CasSingleLogoutClusterFilter.java">CasSingleLogoutClusterFilter</a><p>
 * </p>
 * <p>作用 : 在web服务集群没有使用session同步/设置使用了 会出现在单点登出时 CAS服务器发送的登出请求只会到达到其中的一台web服务器上,其他的服务器上的session仍然有效.
 * 该代码的作用是 在收到登出请求的web服务器上 将收到的这个请求广播到其他的web服务器上
 * </p>
			...
			if ("POST".equals(httpRequest.getMethod())) {
				...
				if (StringUtils.hasText(logoutRequest) && (rebroadcast == null)) {
					try {
						...
						for (Peer peer : this.peers) {//TODO(shisj) 这里可以改造成在zookeeper/redis找到当前register过的web服务器 ip
